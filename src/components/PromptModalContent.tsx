...
export default function PromptModalContent({ prompt, onClose, isFullPage }: PromptModalContentProps) {
  ...
  const [parsedArguments, setParsedArguments] = useState<{name: string, default: string}[]>([]);

  useEffect(() => {
    const argsRegex = /\{argument name="([^"]+)" default="([^"]+)"\}/g;
    const argsMap = new Map<string, string>();
    let match;
    const initial: Record<string, string> = {};
    
    while ((match = argsRegex.exec(prompt.prompt)) !== null) {
      if (!argsMap.has(match[1])) {
        argsMap.set(match[1], match[2]);
        initial[match[1]] = match[2];
      }
    }
    
    const parsed = Array.from(argsMap.entries()).map(([name, def]) => ({ name, default: def }));
    
    // Also use explicit arguments if they exist in the object
    if (prompt.arguments) {
  ...
  }, [prompt.prompt, prompt.arguments]);

  const hasImages = prompt.images && prompt.images.length > 0;

  const getProcessedPrompt = () => {
    let processed = prompt.prompt;

    // 1. Replace verbose syntax: {argument name="X" default="Y"}
    processed = processed.replace(/\{argument name="([^"]+)" default="([^"]+)"\}/g, (match, name, defaultValue) => {
      return customArgs[name] !== undefined && customArgs[name] !== "" ? customArgs[name] : defaultValue;
    });

    // 2. Replace simple syntax for any explicitly defined arguments: {name} or [insert name]
    if (prompt.arguments) {
      prompt.arguments.forEach(arg => {
        const val = customArgs[arg.name] !== undefined && customArgs[arg.name] !== "" ? customArgs[arg.name] : arg.default;
        
        // Replace {name}
        const braceRegex = new RegExp(`\\{${arg.name}\\}`, 'g');
        processed = processed.replace(braceRegex, val);
        
        // Replace [insert name]
        const bracketRegex = new RegExp(`\\[insert ${arg.name}\\]`, 'gi');
        processed = processed.replace(bracketRegex, val);
      });
    }
            ...
            {prompt.images.length > 1 && (
              <div className="flex flex-wrap gap-4">
                {prompt.images.map((img, idx) => (
                  ...
                  >
                    <img src={img} className="w-full h-full object-cover" alt={`${prompt.title} view ${idx + 1}`} />
                  </button>
                ))}
              </div>
            )}
