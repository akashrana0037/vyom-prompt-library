...
export default function HomeClient() {
  ...
  const filteredPrompts = useMemo(() => {
    return (promptsData as any[]).filter((p) => {
      ...
      if (!matchesCategory) {
        const mappedCats = categoryMapping[selectedCategory] || [selectedCategory];
        matchesCategory = mappedCats.includes(p.category) || p.category === selectedCategory;
      }
      ...
      <main className="max-w-[1920px] mx-auto px-6 py-8">
        ...
        <div className="flex items-center gap-1 mb-8 overflow-x-auto pb-4 no-scrollbar border-b border-white/5">
          <button
            ...
            className={`px-4 py-2 text-sm font-bold transition-all shrink-0 rounded-full ${
